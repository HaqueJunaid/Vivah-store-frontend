...
const EditProductPage: React.FC = () => {
    ...
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        const matchedCategory = allCategories.find(
          (item) => item.title.toLowerCase() === prod.category?.toLowerCase()
        )?.title || "Assets";
        ...
        const matchedSubCategory = categoryItem?.baseItems?.find(
          (sub) => sub.title.toLowerCase() === prod.subCategory?.toLowerCase()
        )?.title || prod.subCategory || "";
        ...
        reset({
          ...
          price: prod.price,
          inStock: prod.inStock !== undefined ? prod.inStock : ((prod.quantity ?? 0) > 0),
          description: prod.productInfo?.description || prod.description || "",
          about: prod.productInfo?.about || "",
          note: prod.productInfo?.note || "",
          hasVariants: !!prod.hasVariants,
          variantTitle: prod.variantTitle || "",
          isCustomizable: !!prod.isCustomizable,
          hasFixedQuantities: !!prod.hasFixedQuantities,
        });

        setCustomizationSlots(prod.customizations || []);
        ...
        if (prod.imageUrls && prod.imageUrls.length > 0) {
          ...
          setImagePreviews([prod.imageUrl]);
        } else {
          setImagePreviews([]);
        }

        // Multi-variants pre-population
        if (prod.variants && prod.variants.length > 0) {
          const loaded: FormVariantItem[] = prod.variants.map((v: any, idx: number) => ({
            id: "var-" + (v._id || idx + "-" + Date.now()),
            title: v.title || v.name || `Variant ${idx + 1}`,
            imagePreviews: Array.isArray(v.images) ? v.images : [],
            selectedFiles: [],
            objectUrls: [],
          }));
          setVariantsList(loaded);
        } else if (prod.variantTitle || (prod.variantImages && prod.variantImages.length > 0)) {
          setVariantsList([
            {
              id: "var-" + Date.now(),
              title: prod.variantTitle || "Variant 1",
              imagePreviews: Array.isArray(prod.variantImages) ? prod.variantImages : [prod.variantImages],
              selectedFiles: [],
              objectUrls: [],
            },
          ]);
  ...
  const onSubmit: SubmitHandler<ProductFormInputs> = async (data) => {
    ...
    const remainingExisting = imagePreviews.filter((url) => !url.startsWith("blob:"));
    if (remainingExisting.length === 0 && selectedFiles.length === 0) {
      toast.error("Please retain or upload at least one main product image.");
      return;
    }
    ...
    if (data.hasVariants) {
      ...
      for (let i = 0; i < variantsList.length; i++) {
        ...
        if (v.imagePreviews.length === 0) {
          toast.error(`Please upload or retain at least one image for Variant #${i + 1} (${v.title})`);
          return;
        }
      }
    }
    ...
    if (data.hasVariants) {
      const variantsMeta = variantsList.map((v) => ({
        title: v.title.trim(),
        name: v.title.trim(),
        existingImages: v.imagePreviews.filter((url) => !url.startsWith("blob:")),
      }));
      formData.append("variants", JSON.stringify(variantsMeta));

      // Append new files for each variant
      ...
      const legacyExisting = variantsList[0]?.imagePreviews.filter((url) => !url.startsWith("blob:")) || [];
      formData.append("variantImages", legacyExisting.join(","));
    }
    ...
    // Main Images: Retained URLs
    const existingMainUrls = imagePreviews.filter((url) => !url.startsWith("blob:"));
    existingMainUrls.forEach((url) => {
      formData.append("existingImageUrls", url);
    });

    // Main Images: New files
    ...
    try {
      ...
      await updateProduct(id, formData, (progressEvent) => {
  ...
  if (loadingProduct) {
    return (
      <div className="min-h-screen bg-stone-50/70 p-8 flex flex-col items-center justify-center gap-3">
        <Loader2 className="w-8 h-8 text-[#E41F66] animate-spin" />
        <p className="text-stone-600 text-sm font-medium">Loading product details...</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-200 pb-5">
          <div className="space-y-1.5 min-w-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-stone-900 tracking-tight">Edit Product</h1>
              <span className="text-xs font-semibold bg-stone-100 text-stone-700 px-2.5 py-0.5 rounded-full border border-stone-200 font-mono shrink-0">
                ID: {id}
              </span>
            </div>
            <p className="text-xs sm:text-sm text-stone-500 max-w-2xl wrap-break-word leading-relaxed">
              Update product specs, structured overview, detailed about information, photography, and variant options.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-stone-200 bg-stone-50/50 px-3.5 sm:px-4 py-3 text-sm text-stone-900 outline-none focus:border-[#E41F66] focus:bg-white transition cursor-pointer disabled:opacity-60"
                >
                  {categories.map((cat) => (
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-xs sm:text-sm font-semibold text-stone-800">
                  Click or drag new images to add to the gallery
                </p>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-stone-200/80 shadow-xs space-y-5 sm:space-y-6 overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-stone-100 gap-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <input
                  type="checkbox"
                  id="editHasVariantsToggle"
                  disabled={isSubmitting}
                  {...register("hasVariants")}
                  className="size-5 rounded-md border-stone-300 text-[#E41F66] focus:ring-[#E41F66] cursor-pointer"
                />
                <label htmlFor="editHasVariantsToggle" className="text-xs sm:text-sm font-semibold text-stone-800 cursor-pointer whitespace-nowrap">
                  Has Variants
                </label>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-stone-200/80 shadow-xs space-y-5 sm:space-y-6 overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-stone-100 gap-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <input
                  type="checkbox"
                  id="editIsCustomizableToggle"
                  disabled={isSubmitting}
                  {...register("isCustomizable")}
                  className="size-5 rounded-md border-stone-300 text-[#E41F66] focus:ring-[#E41F66] cursor-pointer"
                />
                <label htmlFor="editIsCustomizableToggle" className="text-xs sm:text-sm font-semibold text-stone-800 cursor-pointer whitespace-nowrap">
                  Allow Customization
                </label>
              </div>
            </div>
            ...
            {isSubmitting ? (
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <p className="text-xs text-stone-500 text-center sm:text-left">
                  Changes will be saved and immediately updated across the live product catalog.
                </p>
                <div className="flex items-center gap-2.5 sm:gap-3 w-full sm:w-auto shrink-0">
                  <button
                    type="button"
