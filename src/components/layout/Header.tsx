...
const Header: React.FC = () => {
  ...
  return (
    <header className="z-10 relative w-full">
      ...
      {typeof window !== 'undefined' && isMobileSearchOpen && createPortal(
      ...
      {typeof window !== 'undefined' && createPortal(
        <div
          className={`lg:hidden fixed inset-0 z-[9999] transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
          onClick={() => setIsMobileMenuOpen(false)}
          aria-hidden={!isMobileMenuOpen}
        >
          <div className='absolute w-full h-[100dvh] inset-0 bg-black/40' />
          <aside
            className={`absolute top-0 right-0 h-[100dvh] w-4/5 max-w-xs bg-stone-50 shadow-xl transition-transform duration-300 flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
            onClick={(e) => e.stopPropagation()}
            role='dialog'
            aria-modal='true'
          >
            <div className='flex justify-between items-center px-4 py-4 border-stone-200 border-b flex-shrink-0'>
              <span className='font-semibold text-stone-900 text-sm'>Menu</span>
              <button
                className='text-stone-700 cursor-pointer p-1'
                onClick={() => setIsMobileMenuOpen(false)}
            ...
            <nav className='flex-1 overflow-y-auto flex flex-col px-4 py-3 text-stone-800 text-sm z-10'>
            ...
            <div className='flex-shrink-0 z-110 relative flex justify-between items-center px-4.5 py-4 bg-stone-50 border-stone-300 border-t w-full text-stone-800'>
              <Link to={`/profile`} onClick={() => setIsMobileMenuOpen(false)} className='flex items-center gap-2'>
                ...
                <h2 className='font-normal text-lg'>Profile</h2>
              </Link>
              <IoIosArrowUp className={`size-5 transition-transform duration-200 cursor-pointer ${isOpen ? 'rotate-180' : ''}`} onClick={() => { setIsOpen(!isOpen) }} />
            </div>

            {/* Profile dropdown, always overlays above menu and is fully visible */}
            <div className={`w-full px-3 max-w-xs bg-stone-50 absolute left-0 transition-all duration-300 z-[120] ${isOpen ? 'bottom-[73px] opacity-100 pointer-events-auto' : 'bottom-0 opacity-0 pointer-events-none'}`} style={{ boxShadow: isOpen ? '0 8px 32px rgba(0,0,0,0.10)' : 'none' }}>
              {token ?
                <>
                  {isAdmin && (
                    <div className='block z-100 relative hover:bg-stone-50 mb-1 p-2 py-3 border border-stone-300 rounded-md w-full text-sm text-nowrap'>
                      <Link to={`/admin/dashboard`} onClick={() => { setIsMobileMenuOpen(false); setIsOpen(false); }} className='flex items-center gap-2 px-2 py-0.5 rounded-sm cursor-pointer font-medium text-stone-800'>
                  ...
                  <div className='block z-100 relative hover:bg-stone-50 mb-1 p-2 py-3 border border-stone-300 rounded-md w-full text-sm text-nowrap'>
                    <Link to={`/profile`} onClick={() => { setIsMobileMenuOpen(false); setIsOpen(false); }} className='flex items-center gap-2 px-2 py-0.5 rounded-sm cursor-pointer'>
               ...
               : (
                <>
                  <div className='block z-100 relative hover:bg-stone-50 mb-1 p-2 py-3 border border-stone-300 rounded-md w-full text-sm text-nowrap'>
                    <Link to={`/register`} onClick={() => { setIsMobileMenuOpen(false); setIsOpen(false); }} className='flex items-center gap-1 px-2 py-0.5 rounded-sm cursor-pointer'>
                      <FiUserPlus className="size-5" /> Sign Up
                    </Link>
                  </div>
                  <div className='block z-100 relative hover:bg-stone-50 mb-1 p-2 py-3 border border-stone-300 rounded-md w-full text-sm text-nowrap'>
                    <Link to={`/login`} onClick={() => { setIsMobileMenuOpen(false); setIsOpen(false); }} className='flex items-center gap-1 px-2 py-0.5 rounded-sm cursor-pointer'>
