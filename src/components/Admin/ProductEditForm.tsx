...
const ProductEditForm: React.FC<ProductEditFormProps> = ({
  ...
  useEffect(() => {
    if (isOpen && productToEdit) {
      ...
      const matchedSubCategory = categoryItem?.baseItems?.find(
        (sub) => sub.title.toLowerCase() === productToEdit.subCategory?.toLowerCase()
      )?.title || productToEdit.subCategory || "";
  ...
  const onSubmit: SubmitHandler<Product> = async (data) => {
    ...
    try {
      setIsUploading(true);
      setUploadProgress(0);
      await onAddProduct(formData, true, (progress) => {
